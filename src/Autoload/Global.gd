...
var integer_zoom := false: set = set_integer_zoom
...
func notification_label(text: String) -> void:
	var _notif := NotificationLabel.new()
	_notif.text = tr(text)
	_notif.position = main_viewport.global_position
	_notif.position.y += main_viewport.size.y
	control.add_child(_notif)
