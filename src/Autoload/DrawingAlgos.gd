...
func scale_3x(sprite: Image, tol: float = 50) -> Image:
	...
	false # scaled.lock() # TODOConverter3To4, Image no longer requires locking, `false` helps to not break one line if/else, so it can freely be removed
	false # sprite.lock() # TODOConverter3To4, Image no longer requires locking, `false` helps to not break one line if/else, so it can freely be removed
	var a: Color
	var b: Color
	var c: Color
	var d: Color
	...
	false # scaled.unlock() # TODOConverter3To4, Image no longer requires locking, `false` helps to not break one line if/else, so it can freely be removed
	false # sprite.unlock() # TODOConverter3To4, Image no longer requires locking, `false` helps to not break one line if/else, so it can freely be removed
	return scaled
...
func crop_image() -> void:
	...
	for f in Global.current_project.frames:
		for cel in f.cels:
			if not cel is PixelCel:
				continue
			false # cel.image.unlock() # TODOConverter3To4, Image no longer requires locking, `false` helps to not break one line if/else, so it can freely be removed  # May be unneeded now, but keep it just in case
