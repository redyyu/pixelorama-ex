...
func get_nearest_tile(point: Vector2) -> Rect2:
	...
	var final := []
	false # tile_mask.lock() # TODOConverter3To4, Image no longer requires locking, `false` helps to not break one line if/else, so it can freely be removed
	for candidate in candidates:
		var rel_pos = point - candidate.position
		if tile_mask.get_pixelv(rel_pos).a == 1.0:
			final.append(candidate)
	false # tile_mask.unlock() # TODOConverter3To4, Image no longer requires locking, `false` helps to not break one line if/else, so it can freely be removed
