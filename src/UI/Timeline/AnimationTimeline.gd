...
func _on_MergeDownLayer_pressed() -> void:
	...
	for frame in project.frames:
		...
		false # top_image.lock() # TODOConverter3To4, Image no longer requires locking, `false` helps to not break one line if/else, so it can freely be removed
		...
		false # top_image.unlock() # TODOConverter3To4, Image no longer requires locking, `false` helps to not break one line if/else, so it can freely be removed
